...
public class Batch_Merge_search_withspeed_rerank {
...
//        BufferedReader reader = new BufferedReader(new FileReader("D://reason_order24_30/sim_data_24.csv"));//换成你的文件名
        ...
        while((line=reader.readLine())!=null) {
            ...
            Batch_Merge_search_withspeed_rerank example = new Batch_Merge_search_withspeed_rerank();
            ...
            if(his_points_numList_gid.size()>0){
                ...
                his_trj_numList = example.search_his_trj_num(Arrays.asList(his_points_numList_gid.keySet().toArray()), query_lng, query_lat, query_dlng, query_dlat, time_slot, week);
...
//            }
                File newcsv = new File("D://trj_test/24_our_withspeed_shortest_rerank.csv");
               // File newcsv = new File("D://trj_test/sim/sim_24_our_withspeed_fastest.csv");
                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                /*根据排序得到的index，输出最优的上车点规划路径*/
                if(all_rankIndex.length>10) {
                    for (int i = 0; i < 10; i++) {
                        //System.out.println(his_trj_numList.get(String.valueOf(distancelist.get(all_rankIndex[i]).getGid())));
                        bw.write(oid + ";" + query_lat + ";" + query_lng + ";" + distancelist.get(all_rankIndex[i]).getGid() + ";" + time_slot_string + ";" + week_string + ";" + weekend + ";" +
                                ...
                                his_trj_numList.get(String.valueOf(distancelist.get(all_rankIndex[i]).getGid())) + ";" +
                                his_points_numList_gid.get(String.valueOf(distancelist.get(all_rankIndex[i]).getGid())) + ";" +
...
//            GraphHopper hopper = createGraphHopperInstance2("C://Users/fama/Desktop/speed/graph_big.osm (2)_24_43_speed.xml");
            ...
            String map_file = "C:/Users/fama/Desktop/24speed/map/graph_big.osm (2)_24_"+time_slot_string+"_speed.xml";
            hopper.setOSMFile(map_file);
            // specify where to store graphhopper files
            String map_file_cache = "target/routing-graph-cache-fama-weight_fastest24_"+time_slot_string;
