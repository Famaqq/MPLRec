...
public class baseline_trj_true_date {
...
//    BufferedReader reader = new BufferedReader(new FileReader("D://reason_order24_30/for_exp_lt500_wtime_20161124_pro.csv"));//换成你的文件名
        BufferedReader reader = new BufferedReader(new FileReader("D://reason_order24_30/all_25_order_6w.csv"));//换成你的文件名
    ...
    while((line=reader.readLine())!=null)
        {
            ...
            baseline_trj_true_date example = new baseline_trj_true_date();
            /*查询500米内邻近路段到目的地的距离*/
            ...
            /*输出最短距离的实际路径*/
            for (int i = 0; i < 1; i++) {
                File newcsv = new File("D://trj_test/true_data_25/true_data_shortest.csv");
                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                if (distancelist.size() > 0) {
                    //将规划好的距离数据写入csv
                    bw.write(oid +  ";" +  query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getGid() + ";" + time_slot + ";" + week + ";" + weekend + ";" +
...
//            System.out.println("lat:"+ his_trj_numList.values().toString());
            ...
            /*输出最短距离的实际路径*/
            for (int i = 0; i < 1; i++) {
                File newcsv = new File("D://trj_test/true_data_25/true_data_prob.csv");
                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                if (distancelistarrIndex.length > 0) {
                    //将规划好的距离数据写入csv
                    bw.write(oid +  ";" +  query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getGid() + ";" + time_slot + ";" + week + ";" + weekend + ";" +
                            distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getMidlat1() +
                            ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getMidlon1() + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getDistance() + ";" +
...
//                System.out.println(distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getLinestring());
                File newcsv = new File("D://trj_test/true_data_25/true_data_closest.csv");
                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                if (distancelistarr.length > 0) {
                    //将规划好的距离数据写入csv
                    bw.write(oid +  ";" +  query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getGid() + ";" + time_slot + ";" + week + ";" + weekend + ";" +
                            distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getMidlat1() +
                            ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getMidlon1() + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getDistance() + ";" +
    ...
    //                System.out.println(distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getGid()+","+distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getMidlat1()+","+distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getMidlon1()+","+distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getDistance());
    //                System.out.println(distancelist.get(distancelistarrIndex[distancelist.size()-1-i]).getLinestring());
                    File newcsv = new File("D://trj_test/true_data_25/true_data_kmeans.csv");
                    BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                    if (distancelist.size() > 0) {
                        //将规划好的距离数据写入csv
                        bw.write(oid +  ";" +  query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(distancelistarrIndex[distancelist.size() - 1 - i]).getGid() + ";" + time_slot + ";" + week + ";" + weekend + ";" +
...
//            else{
//                File newcsv = new File("D://trj_test/true_data_0407/true_data_kmeans.csv");
//                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
//                //将规划好的距离数据写入csv
