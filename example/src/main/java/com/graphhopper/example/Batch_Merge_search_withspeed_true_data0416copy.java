...
public class Batch_Merge_search_withspeed_true_data0416copy {
    ...
    private static void search_rank(GraphHopper hopper_foot, GraphHopper hopper_car) throws IOException {
        ...
        BufferedReader reader = new BufferedReader(new FileReader("D://rerankTest/test.csv"));//换成你的文件名
       // BufferedReader reader = new BufferedReader(new FileReader("D://reason_order24_30/sim10_12.csv"));//换成你的文件名
        ...
        while((line=reader.readLine())!=null) {
            ...
            Batch_Merge_search_withspeed_true_data0416copy example = new Batch_Merge_search_withspeed_true_data0416copy();
            ...
            if(his_points_numList_gid.size()>0){
                ...
                /*对行程时间排序，并记住排名（升序）*/
                int[] trj_timelistarrIndex = new int[distancelist.size()];
                trj_timelistarrIndex = Arraysort2(trj_timelistarr);
                int[] trj_timelistarr_rank = new int[distancelist.size()];

                /*将上述四个排名的值相加，然后对排名总值排序，并记住排名（升序），总排名值越小越好*/
                ...
                for (int i = 0; i < distancelist.size(); i++) {
                    ...
                    trj_timelistarr_rank[trj_timelistarrIndex[distancelist.size() - 1 - i]] = (i + 1);
                }
                for (int i = 0; i < distancelist.size(); i++) {
                    all_rank[i] = his_points_numarr_rank[i] + his_trj_numarr_rank[i] + distancelistarr_rank[i] + trj_timelistarr_rank[i];
                }
...
//            }
//                File newcsv = new File("D://trj_test/24_our_withspeed_shortest_update.csv");
//                File newcsv = new File("D://trj_test/true_data/our_withspeed_fastest.csv");
//                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
//                /*根据排序得到的index，输出最优的上车点规划路径*/
//                for (int i = 0; i < 1; i++) {
//                    bw.write(oid + ";" + query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(all_rankIndex[i]).getGid() + ";" + time_slot_string + ";" + week_string + ";" + weekend + ";" +
//                            distancelist.get(all_rankIndex[i]).getMidlat1() + ";" + distancelist.get(all_rankIndex[i]).getMidlon1() + ";" +
//                            distancelist.get(all_rankIndex[i]).getDistance() + ";" +
//                            distancelist.get(all_rankIndex[i]).getDistance_first() + ";" + distancelist.get(all_rankIndex[i]).getDistance_second() + ";" +
//                            distancelist.get(all_rankIndex[i]).getTime()/1000 + ";" +
//                            distancelist.get(all_rankIndex[i]).getLinestring());
//                    bw.newLine();//换行
//                    bw.close();
//                }
//                // 排名结束时间
...
//                long time_all = near_etime + his_point_etime + his_trj_etime + distance_etime;
                File newcsv = new File("D://rerankTest/true_data_our4.csv");
                BufferedWriter bw = new BufferedWriter(new FileWriter(newcsv, true));
                File newcsv2 = new File("D://rerankTest/true_data_our_Linestring4.csv");
                BufferedWriter bw2 = new BufferedWriter(new FileWriter(newcsv2, true));
                /*根据排序得到的index，输出最优的上车点规划路径*/
//                    for (int i = 0; i < 1; i++) {
//                        bw.write(oid + ";" + query_lat + ";" + query_lng + ";" + query_dlat + ";" + query_dlng + ";" + distancelist.get(all_rankIndex[i]).getGid() + ";" + time_slot_string + ";" + week + ";" + weekend + ";" +
//                                distancelist.get(all_rankIndex[i]).getMidlat1() + ";" + distancelist.get(all_rankIndex[i]).getMidlon1() + ";" +
//                                distancelist.get(all_rankIndex[i]).getDistance() + ";" +
//                                distancelist.get(all_rankIndex[i]).getDistance_first() + ";" + distancelist.get(all_rankIndex[i]).getDistance_second() + ";" +
//                                distancelist.get(all_rankIndex[i]).getTime()/1000 + ";" +
...
//                        bw.close();
//                    }
                for (int i = 0; i < all_rankIndex.length; i++) {
                    bw.write(oid + "," + query_lat + "," + query_lng + "," + query_dlat + "," + query_dlng + "," + distancelist.get(all_rankIndex[i]).getGid() + "," + time_slot_string + "," + week + "," + weekend + "," +
                            distancelist.get(all_rankIndex[i]).getMidlat1() + "," + distancelist.get(all_rankIndex[i]).getMidlon1() + "," +
                            distancelist.get(all_rankIndex[i]).getDistance()+","+distancelistarr_rank[all_rankIndex[i]] + "," +
                            distancelist.get(all_rankIndex[i]).getDistance_first() + "," + distancelist.get(all_rankIndex[i]).getDistance_second() + "," +
                            distancelist.get(all_rankIndex[i]).getTime()/1000+","+trj_timelistarr_rank[all_rankIndex[i]]+","+his_points_numarr[all_rankIndex[i]]+","+his_points_numarr_rank[all_rankIndex[i]]+","+his_trj_numarr[all_rankIndex[i]]+","+his_trj_numarr_rank[all_rankIndex[i]]);
                    bw.newLine();//换行
                    bw2.write(oid + ";" + time_slot_string + ";" + week + ";" + weekend + ";" + distancelist.get(all_rankIndex[i]).getLinestring());
                    bw2.newLine();//换行
                }
                bw.close();
                bw2.close();
...
//            GraphHopper hopper = createGraphHopperInstance2("C://Users/fama/Desktop/speed/graph_big.osm (2)_24_43_speed.xml");
...
//            String map_file = "C:/Users/fama/Desktop/24speed/map_low/graph_big.osm (2)_24_"+time_slot_string+"_speed_update_low.xml";
            ...
            String map_file_cache = "target/routing-graph-cache-fama-weight_fastest_update_map_update_15_"+time_slot_string;
...
//            GHResponse rsp = hopper.route(new GHRequest(Double.valueOf(midlat),Double.valueOf(midlon), Double.valueOf(query_dlat), Double.valueOf(query_dlng))
//                    .setProfile("car"));
